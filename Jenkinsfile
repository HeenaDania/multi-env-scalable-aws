            ...
            steps {
                dir(env.WORKING_DIR) {
                    script {
                        ...
                        // Handle plan exit codes: 0 (no changes), 2 (changes), 1 (error)
                        def planStatus = bat(script: 'terraform plan -out=tfplan -detailed-exitcode', returnStatus: true)
                        if (planStatus != 0 && planStatus != 2) {
                            error "Terraform plan failed with exit code ${planStatus}"
                        } else {
                            echo "Terraform plan completed with exit code ${planStatus} (0: no changes, 2: changes present)"
                        }
            ...
            steps {
                dir(env.WORKING_DIR) {
                    script {
                        ...
                        def destroyStatus = bat(script: 'terraform plan -destroy -out=destroy-plan', returnStatus: true)
                        if (destroyStatus != 0 && destroyStatus != 2) {
                            error "Terraform destroy plan failed with exit code ${destroyStatus}"
                        } else {
                            echo "Terraform destroy plan completed with exit code ${destroyStatus}"
                        }
